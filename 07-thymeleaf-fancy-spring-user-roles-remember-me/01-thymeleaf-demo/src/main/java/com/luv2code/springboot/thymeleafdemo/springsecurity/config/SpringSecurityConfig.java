...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/customers/user/**").hasRole("ADMIN")
		//.antMatchers("/customers/password/**").hasRole("ADMIN")
		...
		.and()
		.rememberMe().key("uniqueAndSecret").tokenValiditySeconds(86400); //the time the token is valid can be configured from the default of 2 weeks to – for example – one day using tokenValiditySeconds(..):
